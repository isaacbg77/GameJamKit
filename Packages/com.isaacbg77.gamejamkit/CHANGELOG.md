...
- `GameJamKit.Sound` — ISoundService/SoundManager, IMusicService/MusicManager with vanilla Awaitable cross-fade, AudioMixerSlider
- `GameJamKit.UI` — IInitializableUI, GameVersionUI, MainMenu/BackToMainMenuScript/InstructionsMenu/WinMenu scripts
...
- MusicManager: dropped `Singleton<MusicManager>` base in favor of `IMusicService` scene-service pattern, matching `SoundManager`/`ISoundService`. Consumers should resolve via `InterfaceFinder.FindInterfaceByType<IMusicService>()` instead of `MusicManager.Instance`.
